...
void OutputWriterText::writeFile(double currentTime)
{
  ...
  for (int i = -1; i < discretization_->u().x_max()-1; i++)
  {
  ...
  for (int j = discretization_->u().y_max()-1; j >= 0; j--)
  {
    file << std::setw(fieldWidth) << j-1 << "|";
    //for (int i = discretization_->uIBegin(); i < discretization_->uIEnd(); i++)
    for (int i = 0; i < discretization_->u().x_max(); i++)
    {
  ...
  for (int i = -1; i < discretization_->v().x_max()-1; i++)
  {
    file << std::setw(fieldWidth) << i;
  }
  ...
  // write v values
  for (int j = discretization_->v().y_max()-1; j >= 0; j--)
  {
    file << std::setw(fieldWidth) << j-1 << "|";
    for (int i = 0; i < discretization_->v().x_max(); i++)
    {
  ...
  file << "p (" << discretization_->p().size()[0] << "x" << discretization_->p().size()[1] << "): " << std::endl 
    << std::string(fieldWidth, ' ') << "|";
  for (int i = -1; i < discretization_->p().x_max()-1; i++)
  {
    file << std::setw(fieldWidth) << i;
  }
  ...
  // write p values
  for (int j = discretization_->p().y_max()-1; j >= 0; j--)
  {
    file << std::setw(fieldWidth) << j-1 << "|";
    for (int i = 0; i < discretization_->p().x_max(); i++)
    {
