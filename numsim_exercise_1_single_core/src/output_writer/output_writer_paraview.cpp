...
OutputWriterParaview::OutputWriterParaview(std::shared_ptr<Discretization> discretization, std::array<double, 2> meshwidth, std::array<int, 2> nCells)
    : OutputWriter(discretization), dx{ meshwidth[0] }, dy{ meshwidth[1] }, size{ nCells[0] + 2,nCells[1] + 2 }
{
  // Create a vtkWriter_
...
void OutputWriterParaview::writeFile(double currentTime)
{
  ...
  // setup for interpolation settings
  enum Directions {
      HORIZONTAL,
      VERTICAL,
      HORIZONTAL_VERTICAL,
  ...
  for (int j = 0; j < size[1] - 1; j++)
  {
    ...
    for (int i = 0; i < size[0] - 1; i++, index++)
    {
      ...
      //arrayPressure->SetValue(index, discretization_->p().interpolateAt(x,y));
        arrayPressure->SetValue(index, discretization_->p().interpolate(i, j, HORIZONTAL_VERTICAL));
    }
  }
  ...
  for (int j = 0; j < size[1] - 1; j++)
  {
    ...
    for (int i = 0; i < size[0] - 1; i++, index++)
    {
      ...
      velocityVector[0] = discretization_->u().interpolate(i, j, VERTICAL);
      velocityVector[1] = discretization_->v().interpolate(i, j, HORIZONTAL);
